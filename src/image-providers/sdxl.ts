...
const workflowFileName = "sdxl.json";
...
export class SDXLProvider implements ImageProvider {
  ...
  async generateImage(
    ...
    if (!this.client) {
      throw new Error(
        "COMFYUI_URL is not configured - the SDXL image provider needs a running ComfyUI server",
      );
    }
    return this.client.runWorkflow(
      this.workflowPath,
      {
        ...
        ...(options.negativePrompt
          ? { "Negative Prompt": { text: options.negativePrompt } }
          : {}),
        ...(options.loraPath
